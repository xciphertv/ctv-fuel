...
local lastFuelUpdate = 0 -- Timestamp of last fuel update
local pendingFuelSync = false -- Flag to indicate if we need to sync with server

-- Security enhancement: Track fuel state locally but verify with server
...
function GetFuel(vehicle)
    if Config.FuelDebug then print("Getting fuel for vehicle", vehicle) end
    
    -- Check if we have locally cached fuel state first
    local plate = GetVehicleNumberPlateText(vehicle)
    if vehicleFuelStates[plate] then
        -- Return cached value for better performance
        return vehicleFuelStates[plate]
    end
    
    -- Fall back to decor value if no cached state
    local fuel = DecorGetFloat(vehicle, Config.FuelDecor)
    
    -- If fuel is invalid, initialize with server value or fallback default
    if fuel <= 0 or fuel > 100 then
        -- Request server sync for this vehicle
        TriggerServerEvent('cdn-fuel:server:RequestFuelSync', plate)
        -- Return a default value temporarily
        fuel = math.random(20, 80)
    end
...
function SetFuel(vehicle, fuel)
    ...
    if type(fuel) ~= 'number' then 
        if Config.FuelDebug then print("Invalid fuel value type:", type(fuel)) end
        return 
    end
    ...
    -- Get the plate for server synchronization
    local plate = GetVehicleNumberPlateText(vehicle)
    if not plate or plate == "" then 
        if Config.FuelDebug then print("Failed to get plate for vehicle:", vehicle) end
        return 
    end
    ...
    -- Only update the vehicle if we're in it or very close
    if IsPedInVehicle(PlayerPedId(), vehicle, false) or #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(vehicle)) < 5.0 then
        SetVehicleFuelLevel(vehicle, fuel + 0.0)
        DecorSetFloat(vehicle, Config.FuelDecor, fuel)
    end
    
    -- Rate-limit server sync to prevent spam
    local currentTime = GetGameTimer()
    if pendingFuelSync or (currentTime - lastFuelUpdate) > 5000 then
        TriggerServerEvent('cdn-fuel:server:UpdateFuel', plate, fuel)
        lastFuelUpdate = currentTime
        pendingFuelSync = false
    else
        pendingFuelSync = true
    end
end
...
local function HandleFuelConsumption(vehicle)
    if not vehicle or IsEntityDead(vehicle) then return end

    local plate = GetVehicleNumberPlateText(vehicle)
    if not plate or plate == "" then return end

    -- Initialize fuel state if needed
    if not vehicleFuelStates[plate] then
        vehicleFuelStates[plate] = math.random(20, 80)
        TriggerServerEvent('cdn-fuel:server:RequestFuelSync', plate)
    end

    -- Skip consumption for blacklisted vehicles
    if IsVehicleBlacklisted(vehicle) then return end

    -- Only consume fuel if engine is running
    if IsVehicleEngineOn(vehicle) then
        local currentFuel = vehicleFuelStates[plate]
        if currentFuel <= 0 then
            -- Force engine off if out of fuel
        ...
        local consumption = Config.FuelUsage[rpmValue] or Config.FuelUsage[0.0]
        local adjustment = Config.Classes[GetVehicleClass(vehicle)] or 1.0
        
        -- Apply consumption
        local fuelToRemove = (consumption * adjustment) / 10
        local newFuel = math.max(0, currentFuel - fuelToRemove)
        
        -- Update only if substantial change occurred (optimization)
        if math.abs(currentFuel - newFuel) > 0.1 then
...
-- Security enhancement: Send start refuel event to server
function StartRefuel(vehicle, fuelAmount, purchaseType)
    local plate = GetVehicleNumberPlateText(vehicle)
    local currentFuel = GetFuel(vehicle)
    
    -- Rate limiting
    local currentTime = GetGameTimer()
    if currentTime - lastRefuelTime < 2000 then
        QBCore.Functions.Notify("Please wait before refueling again", "error")
        return false
    end
    lastRefuelTime = currentTime
    
    -- Validation
    if refuelInProgress then
        QBCore.Functions.Notify("Refueling already in progress", "error")
        return false
    end
    
    -- Notify server about refuel start
    TriggerServerEvent('cdn-fuel:server:StartRefuel', plate, currentFuel, fuelAmount, purchaseType)
    refuelInProgress = true
    
    return true
end

-- Security enhancement: Complete refuel event
function CompleteRefuel(vehicle, addedFuel, purchaseType)
    local plate = GetVehicleNumberPlateText(vehicle)
    local finalFuel = GetFuel(vehicle) + addedFuel
    
    -- Cap at 100
    finalFuel = math.min(100, finalFuel)
    
    -- Update server about completed refuel
    TriggerServerEvent('cdn-fuel:server:CompleteRefuel', plate, finalFuel, addedFuel, purchaseType)
    refuelInProgress = false
    
    return finalFuel
end

-- Security enhancement: Cancel refuel event
function CancelRefuel()
    if refuelInProgress then
        TriggerServerEvent('cdn-fuel:server:CancelRefuel')
        refuelInProgress = false
    end
end
-- Thread Stuff --
...
-- Security enhancement: Server sync events
RegisterNetEvent('cdn-fuel:client:SyncFuel', function(plate, fuel)
    if not plate or not fuel then return end
    
    -- Update cached state
    vehicleFuelStates[plate] = fuel
    
    -- Find vehicle with this plate and update it
    local vehicles = GetGamePool('CVehicle')
    for _, vehicle in ipairs(vehicles) do
        if GetVehicleNumberPlateText(vehicle) == plate then
            -- Only update visual state if we're close or in vehicle
            if IsPedInVehicle(PlayerPedId(), vehicle, false) or #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(vehicle)) < 50.0 then
                SetVehicleFuelLevel(vehicle, fuel + 0.0)
                DecorSetFloat(vehicle, Config.FuelDecor, fuel)
            end
            break
...
-- Security enhancement: Payment response handler
RegisterNetEvent('cdn-fuel:client:PaymentComplete', function(success, remainingFuel)
    if success then
        if refueling then
            -- Continue with refueling process
            QBCore.Functions.Notify("Payment processed successfully", "success")
        else
            -- Handle one-time fuel payments
            QBCore.Functions.Notify("Fuel payment complete", "success")
        end
    else
        -- Handle payment failure
        QBCore.Functions.Notify("Payment failed", "error")
        -- Stop any refueling in progress
        if refueling then
            refueling = false
...
CreateThread(function()
    ...
    while true do
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            -- Only handle fuel for driver
            if GetPedInVehicleSeat(vehicle, -1) == ped then
                HandleFuelConsumption(vehicle)
            end
        else
            -- Update less frequently when not in a vehicle
            Wait(2000)
        end
    end
end)
...
RegisterNetEvent('cdn-fuel:client:RefuelVehicle', function(data)
    ...
    if not holdingnozzle and RefuelingType == nil then 
        QBCore.Functions.Notify("You need the fuel nozzle to refuel", "error")
        return 
    end
    ...
    -- Get the fuel price based on station or type
    ...
    if Config.EmergencyServicesDiscount['enabled'] then
        -- Discount logic here (kept from original)
        -- ...
    end
    ...
    -- Security enhancement: Start refueling process with server validation
    if not StartRefuel(vehicle, amount, purchasetype) then
        return
    end
    ...
    if inGasStation then
        if IsPlayerNearVehicle() then
            ...
            -- Optional: Face vehicle
            if Config.FaceTowardsVehicle and RefuelingType ~= 'special' then
                local bootBoneIndex = GetEntityBoneIndexByName(vehicle, 'boot')
                local vehBootCoords = GetWorldPositionOfEntityBone(vehicle, bootBoneIndex)
                TaskTurnPedToFaceCoord(ped, vehBootCoords, 500)
                Wait(500)
            end
            ...
            CreateThread(function()
                while refueling do
                    ...
                    if Cancelledrefuel then
                        ...
                        -- Security enhancement: Server handles the payment logic
                        TriggerServerEvent('cdn-fuel:server:PayForFuel', finalrefuelamount, purchasetype, FuelPrice, false, CachedFuelPrice)
                        
                        -- Update fuel level
                        local curfuel = GetFuel(vehicle)
                        local finalfuel = CompleteRefuel(vehicle, finalrefuelamount, purchasetype)
            ...
            if Config.Ox.Progress then
                if lib.progressCircle({
                    ...
                    -- Process payment
                    TriggerServerEvent('cdn-fuel:server:PayForFuel', amount, purchasetype, FuelPrice, false, CachedFuelPrice)
                    
                    -- Update fuel level
                    local finalfuel = CompleteRefuel(vehicle, fuelamount, purchasetype)
                    
                    -- Update station reserves if needed
                    if RefuelingType == nil and Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
                ...
                -- QB-ProgressBar logic (with similar security enhancements)
                -- ...
            end
        end
    else
        QBCore.Functions.Notify("You must be at a gas station to refuel", "error")
        return
    end
end)
